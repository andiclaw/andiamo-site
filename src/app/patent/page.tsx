...
export const metadata = {
  title: 'Patent',
  description:
    'US Patent 12,567,119 B1, Autonomous Transportation Systems and Methods. The smart-contract settlement architecture behind Andiamo Tech, Inc.\'s mobility product.',
};
...
const MECHANISMS = [
  { title: 'Smart-contract bid matching', body: 'The user posts trip parameters; vehicles bid in a structured auction; the smart contract sorts and presents bids by user-defined priority; the user selects one; the agreement and its settlement terms are written immutably on-chain. It replaces both the centralized matching algorithm (Uber/Lyft) and the manual booking flow (taxi dispatch) with a deterministic, auditable auction.' },
  ...
  { title: 'Automatic four-way payment split', body: 'Each fare splits 90% operator / 6% city of origin / 2% helper fund / 2% platform maintenance at settlement time, on-chain, with no manual reconciliation. The city-of-origin slice gives municipalities a stake in the arrangement.' },
  ...
  { title: 'Multi-vehicle hail / linked-trip chaining', body: 'Trips that span multiple vehicles (for example car, light rail, then car) are bid and settled as a single linked transaction. Transit-station handoffs and schedule-based vehicles participate in the same auction model.' },
    ...
    <div className="max-w-3xl mx-auto px-6 py-16">
      ...
      <Reveal>
        <section className="space-y-6 text-slate-300 leading-relaxed mb-12">
          ...
          <p className="text-base">
            A smart-contract settlement architecture for mobility. Riders post trip parameters (origin, destination, time window, vehicle preferences), vehicles submit bids (price-per-mile, ETA, vehicle class), the smart contract sorts and presents bids, the rider selects, and an immutable record of the agreement is written to the blockchain. The fare splits automatically across the operator, the city of origin, a helper fund that subsidizes verified low-income rides, and platform maintenance, with no manual reconciliation, escrow agent, or monthly settlement run.
          </p>
        </section>
      </Reveal>
      ...
      <section className="mb-12">
        <Reveal>
          ...
          <p className="text-sm text-slate-400 leading-relaxed mb-5">
            A few of the protected mechanisms carry most of the strategic weight. These are the ones a competitor would have to design around, or license.
          </p>
        </Reveal>
      ...
      <Reveal>
        <div className="flex flex-wrap gap-3 pt-6 border-t hairline">
          ...
          <a href={`mailto:${COMPANY.supportEmail}?subject=Patent%20licensing%20inquiry`} className="focusable px-5 py-2.5 rounded-pill text-sm font-semibold border hairline-bold text-slate-300 hover:text-white transition-all">
            Licensing inquiry →
          </a>
        </div>
      </Reveal>
